...
const hundredsRule = number => {
  ...
  if (
    index >= 0 &&
    numberUnits.length === 3 &&
    number !== '000' &&
    parseInt(number, radix) % 100 === 0
  ) {
...
const tensRule = number => {
  ...
  const tensNumber = parseInt(
    `${numberUnits[numberUnits.length - 2]}${
      numberUnits[numberUnits.length - 1]
    }`,
    radix,
  )
  ...
  if (tensDigit >= 2) {
    const thirdDigit =
      parseInt(numberUnits[numberUnits.length - 1], radix) !== 0
        ? ones[parseInt(numberUnits[numberUnits.length - 1], radix) - 1]
        : ''
